...
async def get_analytics(background_tasks: BackgroundTasks):
    ...
    New additive fields (old frontends safely ignore unknown keys):
    ...
    analytics_payload = {
        # --- Original Agent3Analytics fields (exact same key names + structure) ---
...
async def force_regenerate_analytics(background_tasks: BackgroundTasks):
    ...
    # Increment datasetVersion to guarantee the worker considers this a new version,
    # even if the underlying ticket data hasn't changed since the last run.
    try:
    ...
    # Enqueue the background worker — caller gets the 202 response immediately
    background_tasks.add_task(analytics_service.maybe_regenerate)
